...
class BuildTranslationDataset:
    ...
    def build_decoder_only_dataset(self, pairs, max_length=1024, sep_token='<sep>', inference=False):
        print("Start build decoder-only dataset!")
        dataset = []
    ...
    def test_inference(self, pairs):
        ...
        ed_dataset = self.build_encoder_decoder_dataset(pairs, max_length=32)
        ...
        do_dataset = self.build_decoder_only_dataset(pairs, max_length=32)
        ...
        do_dataset = self.build_decoder_only_dataset(pairs, max_length=32, inference=True)
        for i, sample in enumerate(do_dataset):
            print(f"Sample {i}:")
            print('input_ids:', sample['input_ids'])
            print('attention_mask:', sample['attention_mask'])
